...
- **2.8.7:** two things together. (a) The two-stage verdict — "Can I refinance?" (max supportable loan, all three limits, ≥ combined payoff) then "Should I refinance?" (don't if the total cost of the same money is higher), hiding the proposed loan + schedule (and the Save button) unless both are yes. (b) The Underwriting roll-up auto-loads every property's T12 from disk once on first view — the "Load every property's T12" button is gone, replaced by a passive status line.
- **2.8.8 (final version of the rebuild — folds in what was 2.8.9, plus a NOI fix):**
  - (a) A **Save button** in the Underwriting tab (one save point): commits the property's inputs, recomputes both NOIs from them, and persists Claude's "what to push" into general-data.json. Claude's push now carries **two impact figures per move** (in-place + underwritten dollars) and is fed the property **address** for location-aware ranking; it regenerates on Save-of-changed-inputs / re-analyse / new T12.
  - (b) The push moves become **checkboxes in the refinance calculator**: ticking one adds its impact to the working NOI on the active basis and re-drives the two-stage Can-I / Should-I verdict live (a what-if, never saved).
  - (c) **Lease-up / partial-year NOI fix:** the "last 3 months × 4" annualization now fires for any statement with fewer than 12 dated months (not only a lease-up that starts with two empty months), and it annualizes **both** the in-place AND the underwritten NOI — reserves stay annual, vacancy/management ratios preserved. Fixes understated NOIs on partial-year properties (e.g. Avalon White Plains) that flowed into general-data.json and the whole refinance calculator.
...
- **The proposed loan is priced honestly (v2.8.3).** The refinance calculator no longer fabricates a rate.
  ...
  the comp-derived margin (unchanged, now editable) on the floating side. The rate is spelled out: index +
  spread − tier discount, floored at 0. Removed the calibrated fixed-side spread, the 25 bps floor, the
  automatic Treasury↔SOFR "cheaper wins" switch, and the assumed 35 bps cap cost (now floating-only, default
  0). The credit block shrank to an honest tier readout (the grade + the bps it shaves off your spread). The
  verdict still uses the old payment logic — that is 2.8.5's job. Verified with a new honest-pricing e2e
  (toggle default, index picker, spread default 0, live re-price, the −40 bps Tier 4 discount, floating cap
  cost default 0, no fabricated market rate / auto-switch) plus the refi-touching suites.
