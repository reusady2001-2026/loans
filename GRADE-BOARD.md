...
| P3 | Upload → property | D | **A ✓ done** | A — re-checked A after the ties-gating add-on: gate mutation-sensitive (gate-drop mutant fails), additive fields guarded, Crest byte-identical | 462 ok |
...
**ALL 14 ASPECTS AT A** — gate met on each: automated tests green AND independent adversarial critic signed off. Full-suite verification + release (v2.7.0) pending.