...
| P3 | Upload → property | D | **B** (re-check) → test fix in flight | module A-quality; suite red only from a CAB fixture assumption invalidated by the new classifier — dynamic-code fix with builder; glue fixes verified | 390/8 (fixture) → pending; 7/7 mutants killed; Crest ties |
