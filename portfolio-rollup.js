...
(function (root, factory) {
  ...
  // null. Only ISO strings count — that is the app's stored format (isoToParts);
  // anything else is not a date this app knows. Ties keep the first (senior).
  function earliest(group){
    var best = null, bestT = Infinity;
    group.forEach(function (l){
      var s = l && l.maturityDate, m = (typeof s === "string") && s.match(ISO_DAY);
      if (!m) return;
      var t = Date.parse(m[0]);
  ...
  function rowFor(key, rec, group, hooks, gd, C){
    var senior = group[0] || null;
    var hasLines = !!(rec && rec.lines && Object.keys(rec.lines).length);
    var noi = null, uw = null, st;
    if (rec) {
      noi = fin(C.effectiveNOI(rec));
      ...
      if (hasLines) { var d = C.derive(rec, gd); uw = fin(d && d.underwrittenNOI); }
    ...
    return {
      propKey: key,
      name: (rec && rec.propertyName) || (senior && senior.propertyName) || key,   // a row is never anonymous
      units: fin(rec && rec.units),
      noi: noi, uwNoi: uw,
      dscr: fin(st.dscr), dy: fin(st.dy), ltv: fin(st.ltv),
      balance: fin(st.balance), annualDS: fin(st.annualDS),
      maturity: earliest(group),
      loans: group.length
    };
  }

  // Portfolio totals: dollar columns summed (NOI only where a property has one —
  // "unknown" is not zero, so with no NOI anywhere the total stays null), and
  // portfolio coverage read on the sums: DSCR = ΣNOI / Σdebt service,
  // DY = ΣNOI / Σbalance — null, never 0 or NaN, unless both sides are positive.
  function totalsOf(rows){
    var t = { properties: rows.length, loans: 0, noi: null, uwNoi: null, balance: 0, annualDS: 0, dscr: null, dy: null };
    rows.forEach(function (r){
      t.loans += r.loans || 0;
      if (r.noi != null)   t.noi   = (t.noi   == null ? 0 : t.noi)   + r.noi;
    ...
    t.dscr = (t.noi > 0 && t.annualDS > 0) ? t.noi / t.annualDS : null;
    t.dy   = (t.noi > 0 && t.balance  > 0) ? t.noi / t.balance  : null;
    return t;
  }
  ...
  function buildRows(records, loans, hooks, globalDefaults){
    ...
    var rows = keys.map(function (k){ return rowFor(k, recs[k] || null, ordered(groups[k], hooks), hooks, gd, C); });
    rows.sort(byName);
  ...
  function td(s, cls){ return '<td class="py-1 px-2 text-right text-sm tabular-nums ' + (cls || "text-slate-600") + '">' + s + '</td>'; }
  ...
  function render(mountEl, data, opts){
    ...
    var body = rows.map(function (r){
      return '<tr data-op-prop="' + esc(r.propKey) + '" tabindex="0" title="Open ' + esc(r.name) + '" class="border-t border-slate-100 cursor-pointer hover:bg-brand-50/40 focus:bg-brand-50/40 outline-none">' +
        '<td class="py-1 pr-3 text-sm font-semibold text-slate-800 whitespace-nowrap">' + esc(r.name) + '</td>' +
