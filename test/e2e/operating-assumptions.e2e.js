...
// pick Alpha → capRate inherited → change it to 6.25 → badge "override" → Beta still
// "inherited" → back to Alpha, override persisted → Reset → inherited again.
// Nothing touches localStorage directly; all state flows through the app's own UI.
"use strict";
...
const results = [];
let failed = 0;
function check(cond, msg, extra){ if (cond){ results.push("  ok   " + msg); } else { failed++; results.push("  FAIL " + msg + (extra === undefined ? "" : "  — got " + JSON.stringify(extra))); } }
function finish(code){ console.log(results.join("\n")); console.log((results.length - failed) + " passed, " + failed + " failed"); try { fs.rmSync(UDATA, { recursive:true, force:true }); } catch (e) {} process.exit(code); }

// Poll until fn() (evaluated in the page) is truthy; returns the last value or throws with `what`.
async function until(page, fn, what, timeout){
  const t0 = Date.now(); let last;
  while (Date.now() - t0 < (timeout || 8000)){ last = await page.evaluate(fn); if (last) return last; await page.waitForTimeout(100); }
...
async function badge(page, p){ return (await page.textContent(badgeSel(p))).trim(); }
async function waitBadge(page, p, state){
  await until(page, ([sel, st]) => { const b = document.querySelector(sel); return !!b && b.textContent.trim() === st; }.toString().length && (([sel, st]) => { const b = document.querySelector(sel); return !!b && b.textContent.trim() === st; }), "badge " + p + " = " + state).catch(() => {});
}

// Add a loan through the real form — the minimal set validateLoan accepts for a new
// Fixed loan (index defaults to SOFR; the spread prices it).
async function seedLoan(page, name){
  ...
  const outcome = await until(page, () => {
    ...
    return null;
  }, "the Add Loan form to close for " + name);
...
(async () => {
  ...
  }, "#opPropPick to list the two seeded properties", 15000).catch((e) => { throw new Error(e.message + " — is #opPropPick populated from the loans list?"); });
  ...
  check((await page.inputValue(inputSel("sizing.capRate"))) === "5.5", "Alpha: cap rate shows the global default 5.5 (%)", await page.inputValue(inputSel("sizing.capRate")));
  check((await badge(page, "vacancyPct")) === "inherited", "Alpha: vacancy badge starts 'inherited'");
  ...
  await until(page, (sel) => { const b = document.querySelector(sel); return !!b && b.textContent.trim() === "override"; }.bind(null, badgeSel("sizing.capRate")), "Alpha cap rate badge → override").catch(() => {});
  ...
  check((await page.inputValue(inputSel("sizing.capRate"))) === "6.25", "Alpha: cap rate input shows 6.25", await page.inputValue(inputSel("sizing.capRate")));
  check((await badge(page, "vacancyPct")) === "inherited", "Alpha: vacancy still 'inherited' (patch was minimal)", await badge(page, "vacancyPct"));
  check((await badge(page, "sizing.ltvMax")) === "inherited", "Alpha: max LTV still 'inherited'", await badge(page, "sizing.ltvMax"));

  // ---- Beta is untouched -----------------------------------------------------
  await pick(B);
  await page.waitForTimeout(150);
  check((await badge(page, "sizing.capRate")) === "inherited", "Beta: cap rate badge is 'inherited' (override is per property)", await badge(page, "sizing.capRate"));
  check((await page.inputValue(inputSel("sizing.capRate"))) === "5.5", "Beta: cap rate shows the global 5.5", await page.inputValue(inputSel("sizing.capRate")));
  ...
  check((await page.inputValue(inputSel("sizing.capRate"))) === "6.25", "Alpha again: cap rate still 6.25", await page.inputValue(inputSel("sizing.capRate")));

  // ---- Reset → inherit -------------------------------------------------------
  await page.click("#opAssumpReset");
  await until(page, (sel) => { const b = document.querySelector(sel); return !!b && b.textContent.trim() === "inherited"; }.bind(null, badgeSel("sizing.capRate")), "Alpha cap rate badge → inherited after reset").catch(() => {});
  ...
  check((await page.inputValue(inputSel("sizing.capRate"))) === "5.5", "Alpha: cap rate back to the global 5.5 after Reset", await page.inputValue(inputSel("sizing.capRate")));
  ...
  await pick(B); await page.waitForTimeout(100); await pick(A); await page.waitForTimeout(150);
  check((await badge(page, "sizing.capRate")) === "inherited", "Alpha after round trip: still 'inherited' (reset persisted)", await badge(page, "sizing.capRate"));
